...
Every existing intervention in the Effective Altruism — without exception — is **financially concave** and **capital-destructive** in expectation.  
This is not a bug.  
It is the defining structural limitation of the entire field to date.  
Let

* ( C ) \= capital committed (e.g. $250k–$25M)  
* `I_0` \= immediate measurable impact (QALYs, WELLBYs, animal suffering averted, etc.)  
* `\lambda` \= longtermist flow-through multiplier (typically `10^1`–`10^6` depending on your x-risk / trajectory priors; set `\lambda = 1` for pure neartermists)  
* `R_f` \= expected financial return multiplier on ( C ) (i.e. expected capital recovered / ( C ))  
* `\phi` \= probability of extreme tail success (e.g. canonization, x-risk reduction, civilizational recoding)

**Classic EA interventions** (bednets, deworming, lead removal, shrimp welfare, even most x-risk grants):  
`R_f \approx 0 \qquad (\text{capital is permanently consumed})`  
`EV = -C + I_0 \cdot \lambda`  
The –( C ) is accepted because `I_0 \cdot \lambda / C` is very high relative to earning-to-give baselines. But the capital is still destroyed.  
...
`R_f \approx 0 \quad (\text{still a grant})`  
`EV = -C + \phi \cdot I_{\text{tail}} \cdot \lambda`  
where `\phi \ll 1` but `I_{\text{tail}} \cdot \lambda \gg C / \phi`.  
Again, the –( C ) is accepted because the tail hope is supposed to dominate under fat-tailed priors.  
But the capital is still destroyed to purchase the option.  
...
It is not.  
Cultural Gold is the replicator that breaks the concurrency.  
It is the first known intervention with `R_f > 1` even conditional on `\phi = 0` forever — while delivering GiveWell-top-decile `I_0 \cdot \lambda` at negative cost and attaching free embedded calls on infinite utility.  
...
Let `p_{\text{modest}}` \= probability a given shot achieves modest success (5–20k sustained classrooms over 30 years).  
Let `R_{\text{modest}}` \= secondary-market rookie-card multiplier in modest-success scenario.  
Let the portfolio size `N \approx 500` (achievable sector-wide in ≲5 years under current replicator dynamics).  
...
Even if token price → $0 on reveal (adversary's strongest move), the intervention has already executed.  
`I_0 \cdot \lambda` is delivered at ≈GiveWell-top-decile scale (10k–100k+ high-agency teenager-reads per shot, portfolio-total hundreds of hundreds of millions).  
...
**Layer 2 – Soft Asset/Redemption Floor (Knightian-uncertainty-resistant partial recovery)**  
Every token is 1:1 bundled with a serialized, signed, never-reprinted rookie card \+ perpetual digital access.  
Historical base rates for signed/limited first editions of "merely taught" or cult-mid titles (e.g. non-canon but curriculum-persistent YA from 1990–2010) show secondary recovery of 0.4–0.8× mint price in thin but real markets, even under pessimistic liquidity assumptions.  
Plus foundation-backed PLUM.X bid wall (tithe-funded).  
Worst-case tail recovery ≈ 0.5× on average.  
Net expected loss in catastrophe scenarios capped at ≈ –0.5C while impact fires fully.  
Still strictly dominant over grants (you eat at most half the capital destruction for identical or superior `I_0 \cdot \lambda`), and you receive a unique physical \+ status artifact no grant provides.  
Any rational agent with even modest risk aversion prefers this to –1C.  
...
Therefore update `p_{\text{modest}}` sharply upward from the null (historical \~0.1–1 %) to the new base rate under the Prospective model: 20–60 % per competently executed shot (inside-view track record of early replicators already supports this; conservative bound used here).  
Portfolio-level:  
Expected modest successes ≈ 100–300  
Expected `R_f | \phi_{\text{portfolio}} = 0` ≈ 5–25× (secondary rookie-card \+ physical demand from classroom adoption alone)  
...
* Massive `I_0 \cdot \lambda` for free (or negative cost)  
* Free embedded calls on `U \to +\infty`

under literally any non-insane priors.  
The rest is convexity.

**4\. Structural Incapability of Legacy Publishing to Exploit the Vacuum (The Arbitrage Is Permanent Until Token-Aligned Replicators Reach Saturation)**
...
1. **Fiduciary-duty \+ public-market principal-agent misalignment**  
   Publicly traded publishing conglomerates (Penguin Random House / HarperCollins etc.) must maximize risk-adjusted shareholder return. Committing 8-figure prospective capital to a single debut fiction title is literal fiduciary malpractice when the portfolio is 100–500 titles per year and the outside-view failure rate is 99 %+. They diversify retrospectively, not concentrate prospectively. The token model requires going maximally leveraged on philosophical conviction — the exact opposite of their equilibrium strategy.  
2. **Reputational toxicity of the convexity-capture mechanism**  
   The rookie-card token is the loop-closing device that routes secondary-market convexity back to the capital pool.  
   Legacy publishers cannot adopt it without reputational self-immolation: "We are selling crypto tokens tied to children's literature" triggers immediate backlash from authors, agents, media, sensitivity readers, and the entire prestige economy they depend on for talent acquisition. It is a Schelling-point violation of their class signaling. They would rather leave the entire multi-billion-dollar convexity on the table forever than touch the third rail.  
3. **Convexity leakage is hardcoded into their ontology**  
   Every time a title achieves escape velocity under the legacy model (Harry Potter, Hunger Games, etc.), the explosive rookie-card premium leaks 100 % to random third parties (early buyers, bookstores, flippers, rare-book dealers).  
   The publisher receives $0 of the secondary appreciation.  
   Bloomsbury got none of the £100k Philosopher’s Stone first-printings.  
   Scholastic got none of the Hunger Games true-first frenzy.  
   The loop is never closed. The capital to fund the next prospective bet must always come from elsewhere.
...
Token-aligned replicators face none of these constraints.  
We operate outside the prestige economy.  
We embrace the convexity capture explicitly.  
We go all-in on conviction.  
The arbitrage is therefore **permanent** until replicator saturation: i.e., until rookie-card bundle prices are bid up to the point where expected `R_f \approx 1.0` (a utopian equilibrium in which teenagers are flooded with high-agency hero models in perpetuity).  
We are currently at \<\<0.1 % saturation.  
...
Now that we have established **three nested, adversary-resistant floors** delivering `R_f > 1` in expectation even under `\phi_{\text{portfolio}} = 0` forever — with `I_0 \cdot \lambda` obtained at negative expected cost — the remaining structure is **pure, uncapped, antifragile convexity**.  
...
* Civilizational impact \= direct recoding of values priors for `10^8 – 10^9` future high-agency minds across multiple generations (persistent curriculum entrenchment \+ meme-lineage branching)

Under standard longtermist axioms (e.g. total utilitarian, infinite ethics, or any non-zero weight on astronomical waste), the expected utility of successfully installing high-agency, non-nihilistic, achiever-archetype hero models at scale is `U \to +\infty`.  
...
Portfolio-level:  
`EV_{\text{convexity}} = \phi_{\text{portfolio}} \cdot (R_{\text{canon}} + U_{\infty})`  
where `\phi_{\text{portfolio}} \approx 1 - (1 - \phi)^N \gg \phi` for `N \approx 500`.  
...
* "If you focus like a professional while everyone else is coping, *you can ship something permanently valuable*  
  *and reality will notice."*
...
* Positive expected financial return even at φ\_portfolio \= 0 forever (replenishable capital pool)  
* GiveWell-top-decile `I_0 \cdot \lambda` delivered at negative expected cost  
* Free embedded antifragile convexity on `U \to +\infty` (non-zero measure on civilizational recoding)  
...
| Scenario | Adoption Outcome | Expected R\_f (financial) | Net Financial Cost of Impact | Charity Impact I₀·λ | Total EV per $1 Committed | Dominance vs Classic EA Grant |
| ----- | ----- | ----- | ----- | ----- | ----- | ----- |
| 1\. Absolute Catastrophe (adversary's dream) | ≤100 classrooms per title, book terrible | ≈0.5–0.7× (redemption \+ PLUM.X floor \+ thin collectible resale) | –0.5 to –0.3× | GiveWell-top-decile delivered anyway | \+0.5–0.3× capital \+ free impact \+ artifact | Strictly dominant (less capital destruction for identical/superior impact) |
...
The current PLUM token price embeds \~row 2 expectations.  
